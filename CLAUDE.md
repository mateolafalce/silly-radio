...
- **`player.py`** — an external media player (`mpv` → `ffplay` → `cvlc`/`vlc`, in that order of preference) is launched as a subprocess with `start_new_session=True`. `mpv` receives live volume changes through its JSON IPC socket, preserving the stream connection and buffer; `ffplay` and VLC restart with a different volume flag as a compatibility fallback. Pausing still kills the process, which is acceptable for a live stream because resuming reconnects at the current broadcast position. `requested_playing` stores the user's intent, and the `app.py` loop uses it to relaunch the process if the external player exits unexpectedly.
